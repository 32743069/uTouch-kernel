...
static void iep_power_on(void)
{
    ...
    clk_enable(iep_drvdata1->pd_iep);
    clk_enable(iep_drvdata1->aclk_iep);
    clk_enable(iep_drvdata1->hclk_iep);
    clk_enable(iep_drvdata1->aclk_vio1);
#endif
...
#ifdef IEP_CLK_ENABLE
    clk_disable(iep_drvdata1->aclk_iep);
    clk_disable(iep_drvdata1->hclk_iep);
...
static irqreturn_t iep_isr(void *dev_id)
{
    if (atomic_read(&iep_drvdata1->iep_int) > 0) {
        IEP_DBG("%d\n", iep_get_deinterlace_mode((void*)iep_drvdata1->iep_base));
...
static int __devinit iep_drv_probe(struct platform_device *pdev)
{
    struct iep_drvdata *data;
    int ret = 0;

    data = kzalloc(sizeof(struct iep_drvdata), GFP_KERNEL);
    if (NULL == data) {
    ...
    data->aclk_iep = clk_get(NULL, "aclk_iep");
    ...
    data->hclk_iep = clk_get(NULL, "hclk_iep");
    ...
    /* map the memory */
    if (!request_mem_region(pdev->resource[0].start, RK_IEP_SIZE, "iep_io")) {
        IEP_ERR("failed to reserve iep HW regs\n");
        ret = -EBUSY;
        goto err_ioremap;
    }
    data->iep_base = (void *)ioremap_nocache(pdev->resource[0].start, RK_IEP_SIZE);
    ...
    /* request the IRQ */
    ret = request_threaded_irq(data->irq0, iep_irq, iep_isr, 0, "iep", pdev);
    if (ret) {
...
static int iep_drv_remove(struct platform_device *pdev)
{
    ...
    iounmap((void __iomem *)(data->iep_base));

#ifdef IEP_CLK_ENABLE
    if (data->aclk_iep) {
        clk_put(data->aclk_iep);
    }

    if (data->hclk_iep) {
        clk_put(data->hclk_iep);
    }
...
static struct platform_driver iep_driver = {
    .probe		= iep_drv_probe,
    .remove		= __devexit_p(iep_drv_remove),
    .driver		= {
        .owner  = THIS_MODULE,
        .name	= "iep",
    },
