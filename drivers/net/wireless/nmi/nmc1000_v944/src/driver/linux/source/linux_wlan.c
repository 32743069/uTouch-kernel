...
extern void init_customer_gpio(uint32_t protect);
extern int custom_spi_init(void);
extern void custom_chip_sleep_manually(void);
extern void custom_lock_bus(uint32_t is_open);
extern void custom_unlock_bus(uint32_t is_open);
extern void custom_dcdet_to_wifi_int(uint32_t protect);
extern void custom_irqn_to_wifi_int(uint32_t protect);
extern void custom_chip_wakeup(void);
extern void custom_wakeup(uint32_t is_open);
	
extern int tchip_dc_det_irq_init(unsigned int irq_pin, unsigned long irq_flag);
extern int tchip_dc_det_irq_release(unsigned int irq_pin);
...
int reopen=0;
 ...
 #define _linux_wlan_device_removal()           {}
#elif defined(PLAT_RK3026_TCHIP)		// added by wood
...
extern int rk29sdk_wifi_set_carddetect(int val); 
	...
	#else
	#define  _linux_wlan_device_power_off() do { \
		if ( NMC1000_GPIO_POWER_N != INVALID_GPIO ){\
			gpio_set_value(NMC1000_GPIO_POWER_N , !NMC1000_GPIO_POWER_ENABLE_VALUE);\
		}\
		if ( NMC1000_GPIO_RESET_N != INVALID_GPIO ){\
			gpio_set_value(NMC1000_GPIO_RESET_N,!NMC1000_GPIO_RESET_PIN_ENABLE_VALUE);\
		}\
	}while(0);
	#define _linux_wlan_device_power_on()     do { \
		_linux_wlan_device_power_off(); \
		mdelay(400) ;\
		if ( NMC1000_GPIO_POWER_N != INVALID_GPIO ){\
			gpio_request(NMC1000_GPIO_POWER_N , "nmc1000_power");\
			gpio_direction_output(NMC1000_GPIO_POWER_N,NMC1000_GPIO_POWER_ENABLE_VALUE);\
			mdelay(400) ;\
		}\
		if ( NMC1000_GPIO_RESET_N != INVALID_GPIO ){\
			gpio_request(NMC1000_GPIO_RESET_N , "nmc1000_reset");\
			gpio_direction_output(NMC1000_GPIO_RESET_N,NMC1000_GPIO_RESET_PIN_ENABLE_VALUE);\
			mdelay(400) ;\
		}\
	}while(0);
	#endif //endif for NMI_SDIO
#else
 #define _linux_wlan_device_detection() 	{}
 #define _linux_wlan_device_removal()		{}
 #define _linux_wlan_device_power_on()		{}
...
#ifdef DISABLE_PWRSAVE_AND_SCAN_DURING_IP
extern NMI_TimerHandle hDuringIpTimer;
#endif
...
extern void resolve_disconnect_aberration(void* drvHandler);
extern struct semaphore custom_gpio_lock;
...
#define IRQ_NO_WAIT	0
	...
	*/
unsigned int g_mac_open = 0;
static struct semaphore close_exit_sync;
//TCHIP
static struct semaphore close_reent;
unsigned int int_rcvdU;
unsigned int int_rcvdB;
unsigned int int_clrd;
//static struct timer_list custom_gpio_timer;
static struct task_struct* custom_gpio_thread;
...
extern volatile uint32_t gpio_v_shadow;
...
volatile uint8_t screen_off = NMI_FALSE;
static struct wake_lock screen_off_wakelock;

static void screen_early_suspend(struct early_suspend *h)
{
	printk("***** Early susped called ******\n");

	wake_lock_timeout(&screen_off_wakelock, 3 * HZ);
	/* if MAC open deinitIRQ */
	//if(g_mac_open)
...
void custom_bus_resume(void)
{
	tchip_dc_det_irq_release(GPIO_NUM);
...
static void screen_late_resume(struct early_suspend *h)
{
	printk("***** Late resume called ******\n");
	/* 
		- callback to free the gpio
		- Request GPIO
	*/
	#if 0
	tchip_dc_det_irq_release(GPIO_NUM);
	...
	screen_off = NMI_FALSE;
	//custom_gpio_thread = kthread_run(check_DCDET,NULL,"GPIOThread");		

}

 
static struct early_suspend str_nmc_early_suspend = {
	.suspend = screen_early_suspend,
	.resume  = screen_late_resume,
...
extern int Handle_GetRssi(void);
extern void custom_gpio_reprogram(void);
...
static int DebuggingThreadTask(void* vp)
{
	...
	while(bDebugThreadRunning)
	{
		if(g_linux_wlan->nmc1000_initialized)
		{
			...
			{			
				priv = wiphy_priv(g_linux_wlan->strInterfaceInfo[0].nmc_netdev->ieee80211_ptr->wiphy);
				#if 0
				printk(" == get RSSI ==[%d]\n",tst);
				if(Handle_GetRssi())
					rssI_failure_count++;  
				...
				#endif
				if(cfg_timed_out_cnt >= 15) // || !(tst++%20))
				{
					cfg_timed_out_cnt = 0;					
					timeout = 10;
					...
					printk("\n\n<<<<<>>>>>\n\n");
					mac_close(g_linux_wlan->strInterfaceInfo[0].nmc_netdev);
#if 0					
...
#endif
					while(mac_open(g_linux_wlan->strInterfaceInfo[0].nmc_netdev)&&--timeout);
					{
						msleep(100);
					}
					if(timeout == 0)
    ...
    switch (event) {
		...
		case NETDEV_UP:
			...
			if(nic->iftype==STATION_MODE||nic->iftype==CLIENT_MODE)	
			{
				...
				NMI_TimerStop(&hDuringIpTimer, NMI_NULL);
...
uint32_t linux_wlan_get_IRQN_gpio_value(void)
{
...
#elif defined (PLAT_RK3026_TCHIP)
...
static int linux_wlan_init_test_config(struct net_device *dev, linux_wlan_t* p_nic){
	...
	**/
	...
	if (!g_linux_wlan->oup.wlan_cfg_set(0, WID_11N_IMMEDIATE_BA_ENABLED, c_val, 1, 1,(NMI_Uint32)pstrWFIDrv))
		goto _fail_;
...
extern void nmi_wlan_global_reset(void);
...
int mac_close(struct net_device *ndev)
{
	...
	perInterface_wlan_t* nic;
	tstrNMI_WFIDrv * pstrWFIDrv;
	linux_wlan_lock(&close_reent);	
	
	nic = netdev_priv(ndev);

	if( (nic == NULL) || (nic->nmc_netdev == NULL) || (nic->nmc_netdev->ieee80211_ptr == NULL) || (nic->nmc_netdev->ieee80211_ptr->wiphy == NULL) )
    	{
    		PRINT_ER("nic = NULL\n");
    		return 0;
    	}
	...
	if(!(g_linux_wlan->nmc1000_initialized))
	{
		linux_wlan_unlock(&close_reent);
		printk("Mac_Close: already deinint\n");
		return 0;
	}


  
	PRINT_D(GENERIC_DBG,"Mac close %x\n",ndev);
...
//{	
	...
	custom_lock_bus(0);
	nmc1000_hard_reset();
	init_customer_gpio(0);
	custom_unlock_bus(0);
/**Not sure check with ISLAM*/
	custom_gpio_reprogram();
	...
	//AdjustDstAddr(nmc_netdev,buff);
   	...
   	nic = netdev_priv(nmc_netdev);
		...
		if(size > 0){
			...
			if(eth_h->h_proto == 0x8e88)
			{
				NMI_PRINTF("EAPOL RECEIVED NETDEV: %x\n",nmc_netdev);
...
int nmc_netdev_init(void){
	...
	linux_wlan_init_lock("close_reent",&close_reent,1);
...
static int __init init_nmc_driver(void){
...
#if defined (NMC_DEBUGFS)
	if(nmc_debugfs_init() < 0) {		// [[ added by rachel
	...
	wake_lock_init(&screen_off_wakelock, WAKE_LOCK_SUSPEND,"screen_off_wakelock");
	register_early_suspend(&str_nmc_early_suspend);           


	custom_spi_init();
	sema_init((struct semaphore*)&custom_gpio_lock,1);
	//spin_lock_init(&custom_gpio_lock);
	init_customer_gpio(0);
	//custom_gpio_thread = kthread_run(check_DCDET,NULL,"GPIOThread");

	is_mac_opened_yet = g_mac_open;
	custom_lock_bus(is_mac_opened_yet);
	custom_chip_sleep_manually();
...
static void __exit exit_nmc_driver(void)
{
	...
	unregister_early_suspend(&str_nmc_early_suspend);
	wake_lock_destroy(&screen_off_wakelock);
	
	linux_wlan_deinit_lock(&close_exit_sync);
	NMI_FREE(g_linux_wlan);
	linux_wlan_deinit_lock(&close_reent);
