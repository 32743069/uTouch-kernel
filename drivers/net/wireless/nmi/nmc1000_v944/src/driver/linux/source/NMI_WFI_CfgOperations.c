...
void NMI_WFI_p2p_rx (struct net_device *dev,uint8_t *buff, uint32_t size)
{
	...
	//Get NMI header
	NMI_memcpy(&header, (buff-HOST_HDR_OFFSET), HOST_HDR_OFFSET);
			...
			switch(buff[ACTION_SUBTYPE_ID])
			{
				case GAS_INTIAL_REQ:
					...
					case PUBLIC_ACT_VENDORSPEC:
					{
						...
						if(!NMI_memcmp(u8P2P_oui,&buff[ACTION_SUBTYPE_ID+1],4))
						{
							if ((buff[P2P_PUB_ACTION_SUBTYPE] == GO_NEG_REQ ||  buff[P2P_PUB_ACTION_SUBTYPE] == GO_NEG_RSP))
							{
								if(!bNmi_ie)
								{
									...
									{
										if(!NMI_memcmp(u8P2P_vendorspec,&buff[i],6))
										{
							...
							if(u8P2Plocalrandom > u8P2Precvrandom)
							{
									...
									|| buff[P2P_PUB_ACTION_SUBTYPE] == P2P_INV_REQ ||  buff[P2P_PUB_ACTION_SUBTYPE] == P2P_INV_RSP))
								{
									...
									{
								       	if(buff[i]== P2PELEM_ATTR_ID && !(NMI_memcmp(u8P2P_oui,&buff[i+2],4)) )
										{
											NMI_WFI_CfgParseRxAction(&buff[i+6],size-(i+6));
											break;
										}
									}	
								}
						...
						if( (buff[P2P_PUB_ACTION_SUBTYPE] == GO_NEG_REQ ||  buff[P2P_PUB_ACTION_SUBTYPE] == GO_NEG_RSP ) && ( bNmi_ie))
						{
...
{
	NMI_memcpy(buff,u8P2P_vendorspec,sizeof(u8P2P_vendorspec));
}
/**
...
#endif
{
	...
	*cookie = (unsigned long)buf;
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) 
	{
		...
		NMI_memcpy(mgmt_tx->buff,buf,len);
		mgmt_tx->size=len;	
		...
		else if (ieee80211_is_action(mgmt->frame_control)) 
		{
			...
			{
				NMI_PRINTF("PUBLIC ACTION FRAME\n");
				...
				switch(buf[ACTION_SUBTYPE_ID])
				{
					...
					case PUBLIC_ACT_VENDORSPEC:
					{
						/*Now we have a public action vendor specific action frame, check if its a p2p public action frame
						based on the standard its should have the p2p_oui attribute with the following values 50 6f 9A 09*/
						if(!NMI_memcmp(u8P2P_oui,&buf[ACTION_SUBTYPE_ID+1],4))
						{
							/*For the connection of two NMI's connection generate a rand number to determine who will be a GO*/
									...
									|| buf[P2P_PUB_ACTION_SUBTYPE] == P2P_INV_REQ ||  buf[P2P_PUB_ACTION_SUBTYPE] == P2P_INV_RSP))
							{
								if(u8P2Plocalrandom > u8P2Precvrandom)
								{
									...
									{
								       	if(buf[i]== P2PELEM_ATTR_ID && !(NMI_memcmp(u8P2P_oui,&buf[i+2],4)) )
										{
											if(buf[P2P_PUB_ACTION_SUBTYPE]==P2P_INV_REQ || buf[P2P_PUB_ACTION_SUBTYPE]==P2P_INV_RSP)
												NMI_WFI_CfgParseTxAction(&mgmt_tx->buff[i+6],len-(i+6),NMI_TRUE); 
											else
												NMI_WFI_CfgParseTxAction(&mgmt_tx->buff[i+6],len-(i+6),NMI_FALSE); 
											break;
										}
									}
	...
	u8 *mac)
{
	...
	if(nic->iftype == AP_MODE || nic->iftype == GO_MODE)
	{
		...
		if(mac == NMI_NULL)
		{
			...
			s32Error = host_int_del_allstation(priv->hNMIWFIDrv , priv->assoc_stainfo.au8Sta_AssociatedBss);
		}
		else
		{
