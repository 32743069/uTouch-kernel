...
unsigned int g_mptool_running = 0;	/* tony for mp tool */
#ifdef NMC_MP_TOOL_SUPPORT
#include "linux_wlan_cdev.c"	/* tony for mptool test */
#endif
...
static int DebuggingThreadTask(void* vp)
{
	...
	while(bDebugThreadRunning)
	{
		if(g_linux_wlan&&g_linux_wlan->nmc1000_initialized)
		{
			...
			{			
				priv = wiphy_priv(g_linux_wlan->nmc_netdev->ieee80211_ptr->wiphy);
				#if 0
				printk(" == get RSSI ==[%d]\n",tst);
				if(Handle_GetRssi())
					rssI_failure_count++;  
				...
				if(cfg_timed_out_cnt >= 15) // || !(tst++%20))
				{
					...
					printk("\n\n<<<<<>>>>>\n\n");
					mac_close(g_linux_wlan->nmc_netdev);
#if 0					
...
#endif
					while(mac_open(g_linux_wlan->nmc_netdev)&&--timeout)
					{
						msleep(100);
					}
					if(timeout == 0)
...
static void deinit_irq(linux_wlan_t* nic){
	#if (!defined NMI_SDIO) || (defined NMI_SDIO_IRQ_GPIO)
		/* Deintialize IRQ */
		if((&nic->dev_irq_num != 0)){// && screen_off == NMI_FALSE){
...
int wlan_init_locks(linux_wlan_t* p_nic){
	...
	linux_wlan_init_lock("wdt_lock/wdt_ended",&nic->wdt_thread_ended,0);
...
static int wlan_deinit_locks(linux_wlan_t* nic){
	...
	if(&nic->wdt_thread_ended != NULL)
		linux_wlan_deinit_lock(&nic->wdt_thread_ended);
...
int wlan_initialize_threads(linux_wlan_t* nic){
	...
	{		
		...
		nic->wdt_thread= kthread_run(DebuggingThreadTask,(void*)nic,"DebugThread");												
		if(nic->wdt_thread == 0){
...
int mac_open(struct net_device *ndev){    
	...
	g_mac_open = 1;
	up(&custom_gpio_lock);	
	//mod_timer(&custom_gpio_timer, (jiffies + msecs_to_jiffies(GPIO_CHECK_PERIOD)));
	//linux_wlan_lock(&close_exit_sync);
//add by ruan
	//printk(KERN_ERR "########  nmc1000 lock the wake_lock\n");
	//wake_lock(&screen_off_wakelock);
//end
    return 0;
}
#else
...
int mac_close(struct net_device *ndev)
{
	...
	//up(&custom_gpio_lock);
//add by ruan
	//printk(KERN_ERR "########  nmc1000 relesas the wake_lock\n");
	//wake_unlock(&screen_off_wakelock);
//end
	return 0;
}
...
#ifdef NMC_MP_TOOL_SUPPORT
	if(init_cdev() < 0) {
		NMI_PRINTF("fail to create character device\n");
		return -1;
	}
#endif
...
static void __exit exit_nmc_driver(void){
	...
	{
	#ifndef NMI_SDIO
		PRINT_D(INIT_DBG,"<NO> SPI unregsiter...\n");	
