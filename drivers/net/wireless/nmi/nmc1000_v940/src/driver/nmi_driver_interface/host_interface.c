...
#define HOST_IF_MSG_DEL_ALL_STA  ((NMI_Uint16)36)
...
typedef struct 
{
	NMI_Uint8 au8Sta_DelAllSta[MAX_NUM_STA][ETH_ALEN];
	NMI_Uint8 u8Num_AssocSta;
}tstrHostIFDelAllSta;
...
NMI_SemaphoreHandle hSemHostIntDeinit;
NMI_TimerHandle g_hPeriodicRSSI;
	...
	{		
		...
		if(pstrWFIDrv->enuHostIFstate == HOST_IF_WAITING_CONN_RESP)
		{
			   ...
			   (strConnectInfo.u16ConnectStatus == SUCCESSFUL_STATUSCODE))
			{
				...
				NMI_TimerStart(&hDuringIpTimer, 10000, NMI_NULL, NMI_NULL);
				#endif
...
static void Handle_DelAllSta(void * drvHandler,tstrHostIFDelAllSta* pstrDelAllStaParam)
{
	...
	strWID.u16WIDid = (NMI_Uint16)WID_DEL_ALL_STA;
	strWID.enuWIDtype = WID_STR;
	strWID.s32ValueSize =(pstrDelAllStaParam->u8Num_AssocSta * ETH_ALEN) +1;
	
	PRINT_D(HOSTINF_DBG,"Handling delete station \n");
	
	strWID.ps8WidVal = NMI_MALLOC((pstrDelAllStaParam->u8Num_AssocSta * ETH_ALEN)+1);
	if(strWID.ps8WidVal == NULL)
	{
		NMI_ERRORREPORT(s32Error, NMI_NO_MEM);
	}
	...
	for(i=0;i<MAX_NUM_STA;i++)
	{
		if(memcmp(pstrDelAllStaParam->au8Sta_DelAllSta[i],au8Zero_Buff,ETH_ALEN))
			NMI_memcpy(pu8CurrByte, pstrDelAllStaParam->au8Sta_DelAllSta[i], ETH_ALEN);
		else
			continue;
...
{	
	...
	NMI_TimerStart(&(g_hPeriodicRSSI),5000,(void*)pstrWFIDrv,NULL);
}
...
NMI_Sint32 host_int_init(NMI_WFIDrvHandle* phWFIDrv)
{
	...
	*phWFIDrv = (NMI_WFIDrvHandle)pstrWFIDrv;
	...
	{	
		...
		NMI_SemaphoreCreate(&hSemHostIntDeinit, &strSemaphoreAttrs);
	}
	...
	if(clients_count==0)
	{
		...
		s32Error = NMI_TimerCreate(&(g_hPeriodicRSSI),GetPeriodicRSSI, NMI_NULL);
		if(s32Error < 0)
		{
...
NMI_Sint32 host_int_deinit(NMI_WFIDrvHandle hWFIDrv)
{
	...
	NMI_SemaphoreAcquire(&hSemHostIntDeinit, NULL);
	...
	if(NMI_TimerDestroy(&(g_hPeriodicRSSI), NMI_NULL)){
			printk(">> Connect timer is active \n");
			//msleep(HOST_IF_CONNECT_TIMEOUT+1000);
		}
	...
	NMI_SemaphoreRelease(&hSemHostIntDeinit, NULL);
	return s32Error;
}
...
*  @param[in]  NMI_WFIDrvHandle hWFIDrv, NMI_Uint8 pu8MacAddr[][ETH_ALEN]s
*  @return 	   Error code.
*  @author	
*  @date	
*  @version	1.0
*/
NMI_Sint32 host_int_del_allstation(NMI_WFIDrvHandle hWFIDrv, NMI_Uint8 pu8MacAddr[][ETH_ALEN])
{
	NMI_Sint32 s32Error = NMI_SUCCESS;
	tstrNMI_WFIDrv * pstrWFIDrv = (tstrNMI_WFIDrv *)hWFIDrv;
	tstrHostIFmsg strHostIFmsg;
	tstrHostIFDelAllSta* pstrDelAllStationMsg = &strHostIFmsg.uniHostIFmsgBody.strHostIFDelAllSta;
	NMI_Uint8 au8Zero_Buff[ETH_ALEN]={0};
	NMI_Uint32 i;
	NMI_Uint8 u8AssocNumb=0;
	...
	for(i=0;i<MAX_NUM_STA;i++)
	{
		...
		{
			NMI_memcpy(pstrDelAllStationMsg->au8Sta_DelAllSta[i], pu8MacAddr[i], ETH_ALEN);
