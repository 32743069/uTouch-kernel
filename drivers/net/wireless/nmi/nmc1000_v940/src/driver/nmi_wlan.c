...
struct semaphore custom_gpio_lock;
...
extern NMI_Uint8 g_nmc_initialized; //AMR : 0422 RK3026 Crash issue
#endif
...
static uint32_t dbgflag = 0;//N_INIT|N_ERR|N_INTR|N_TXQ|N_RXQ;
		...
		{			
			...
			if (!is_cfg_packet) {
				...
				if (rsp.type == NMI_CFG_RSP) {
					...
					**/
				PRINT_D(GENERIC_DBG,"p->cfg_seq_no = %d - rsp.seq_no = %d\n",p->cfg_seq_no,rsp.seq_no);
					...
					**/
					...
					if (p->indicate_func.mac_indicate)
					{
						PRINT_D(GENERIC_DBG, "Unlocking mac indicate");
...
/*Enable/Disable GPIO configuration for FW logs*/
	...
	nmi_debug(N_ERR, "[nmi start]: set NMI_DISABLE_NMC_UART\n");
#else
	reg &= ~NMI_DISABLE_NMC_UART;
	nmi_debug(N_ERR, "[nmi start]: unset NMI_DISABLE_NMC_UART\n");
#endif
...
void nmi_wlan_global_reset(void)
{

	nmi_wlan_dev_t *p = (nmi_wlan_dev_t *)&g_wlan;
	acquire_bus(ACQUIRE_AND_WAKEUP);
	p->hif_func.hif_write_reg(NMI_GLB_RESET_0,0x0);
	release_bus(RELEASE_ONLY);	
}
...
volatile uint32_t gpio_v_shadow = 0;
extern uint32_t nmc_spi_suspended;
extern volatile int g_bWaitForRecovery;
#define MAC_STATE_CLOSE_OPEN	1
#define MAC_STATE_OPEN_CLOSE	2


void custom_lock_bus(uint32_t is_open)
{
	if(is_open)
	{
		g_wlan.os_func.os_enter_cs(g_wlan.hif_lock);
...
void custom_unlock_bus(uint32_t is_open)
{	
	if(is_open)
	{
		g_wlan.os_func.os_leave_cs(g_wlan.hif_lock);		
...
void custom_wakeup(uint32_t is_open)
{
	if(!is_open)
	{
		custom_chip_wakeup();
...
void custom_dcdet_to_wifi_int(uint32_t protect)
{
	uint32_t reg;
	uint32_t is_mac_open = g_mac_open;

	if(is_mac_open)
		linux_wlan_disable_irq(1);

	if(protect)
	{
		custom_lock_bus(is_mac_open);	
		custom_wakeup(is_mac_open);
	}


	/* MUX IRQN to be GPIO_2, change it to be input */
	custom_spi_read_reg(0x1408, &reg);
	reg &= ~(0x7<<8);
	custom_spi_write_reg(0x1408, reg);

	/* reset GPIO_6 [0] */
	custom_spi_read_reg(0x20100, &reg);
	reg &= ~(0x1<<6);
	custom_spi_write_reg(0x20100, reg);	


	//custom_spi_read_reg(0x20108, &reg);
	//reg &= ~(0x1<<2);
	//custom_spi_write_reg(0x20108, reg);	
	if(protect && !g_mac_open)
	{
...
void custom_irqn_to_wifi_int(uint32_t protect)
{
	uint32_t reg;
	uint32_t is_mac_open = g_mac_open;
	//linux_wlan_disable_irq(1);
...
void nmc1000_gpio_set_value(int gpio_num,int v)
{
	...
	PRINT_D(GENERIC_DBG,">> GPIO_RSET[%d]: %d -mac_open: %d\n",gpio_num,v,is_mac_open);
	//printk(">> GPIO_SET: %d\n",is_mac_open);
	...
	if(g_bWaitForRecovery)
	{
		if(v == 0)
			gpio_v_shadow &= ~(0x1<< gpio_num);
		else
			gpio_v_shadow |= (0x1 << gpio_num);
	...
	custom_lock_bus(is_mac_open);	
	if((is_mac_open != g_mac_open))
	 {
		 mac_state_changed = (is_mac_open)?MAC_STATE_OPEN_CLOSE: MAC_STATE_CLOSE_OPEN;
	 }
...
int nmc1000_gpio_get_value(int gpio_num)
{
	...
	uint32_t is_mac_open = g_mac_open;
	uint32_t mac_state_changed = 0;
	PRINT_D(GENERIC_DBG,">> GPIO_RGET[%d] - mac_open:%d \n",gpio_num,is_mac_open);
	...
	{
		printk(">>Recovering ...\n");
		return ((gpio_v_shadow & (0x1<<gpio_num))? 1: 0);
	}

	if(nmc_spi_suspended)
	{
		printk(">> SPI suspended <Wait for IRQ>\n");
		return -1;
	}
	...
	if(gpio_num == 6)
	{
		custom_dcdet_to_wifi_int(0);
		reg = linux_wlan_get_IRQN_gpio_value();
		custom_irqn_to_wifi_int(0);
		//custom_spi_read_reg(0x20104, &reg);
		//reg &= (0x1<<6);
		//return ((reg)? 1: 0);	
	}
	else
	...
	gpio_v_shadow = (reg)?(gpio_v_shadow |(0x1<<gpio_num)):(gpio_v_shadow &~(0x1<<gpio_num));
	if(!g_mac_open)
		custom_chip_sleep_manually();
	...
	if(mac_state_changed != MAC_STATE_OPEN_CLOSE)
		custom_unlock_bus(is_mac_open);
	PRINT_D(GENERIC_DBG,"<<GPIO_Get [%d]: %d\n",gpio_num,reg);
	return ((reg)? 1: 0);	
}
...
void init_customer_gpio(uint32_t protect)
{
	...
	if(protect)
		...
		printk("\n>> Initializing customer GPIO <<\n");
		custom_spi_read_reg(0x20108, &reg);
		reg |= (0x1 << 1) |(0x1 << 3) |(0x1 << 4) | (0x1 << 6) ;
		custom_spi_write_reg(0x20108, reg);
...
{	
	...
	if( g_wlan.hif_func.hif_read_reg == NULL )
		return 0;
...
int nmi_wlan_init(nmi_wlan_inp_t *inp, nmi_wlan_oup_t *oup)
{
	...
	PRINT_D(INIT_DBG,"Initializing NMI_Wlan ...\n");
