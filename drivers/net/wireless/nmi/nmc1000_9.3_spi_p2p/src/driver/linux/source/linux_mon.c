                                        ...
                                        struct net_device *dev)
{
	...
	/*if((skb->data[14]==0x08) ||(skb->data[14]==0x88) )
	{
			for(i=0;i<skb->len;i++)
				printk("E:%02x\n",skb->data[i]);
	...
	//NMI_PRINTF("MON XMIT : %x, NETDEV:%x\n",skb->data[0],mon_priv->real_ndev);
	...
	{
		//NMI_PRINTF("BROADCAST DEAUTHENTICATION\n");
		/*for(i=0;i<26;i++)
			NMI_PRINTF("DBuf[%d]=%02x\n",i,skb->data[i]);*/
