...
int mac_open(struct net_device *ndev){    
	...
	if(closed)
	{
		NMI_PRINTF("MAC OPEN after close\n");
		closed = 0;
		for(i=0;i<g_linux_wlan->u8NoIfcs;i++)
		{
			if(g_linux_wlan->strInterfaceInfo[i].nmc_netdev != ndev)
			{
				mac_open(g_linux_wlan->strInterfaceInfo[i].nmc_netdev);
			}			
		}		
	}
	#endif
