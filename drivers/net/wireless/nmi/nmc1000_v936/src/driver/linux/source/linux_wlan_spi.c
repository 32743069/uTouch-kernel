...
extern void custom_bus_suspend(void);
extern void custom_bus_resume(void);

uint32_t nmc_spi_suspended = 0;
static int nmc_spi_suspend(void)
{
	printk("\n>>> BUS suspend <<<\n");
	nmc_spi_suspended = 1;
	custom_bus_suspend();
...
static int nmc_spi_resume(void)
{
	printk("\n>>> BUS resume <<<\n");
	custom_bus_resume();
	nmc_spi_suspended = 0;
...
struct spi_driver nmc_bus __refdata = {
		...
		.suspend = nmc_spi_suspend,
		.resume = nmc_spi_resume,
};
